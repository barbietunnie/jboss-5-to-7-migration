	...
	public String parse(MessageBean msgBean) {
		...
		if (msgBean.getRfc822() != null) {
			...
			if (aNode != null) {
				...
				if (!StringUtils.isEmpty(rfcHeaders)) {
					// rfc822 headers
					if (isDebugEnabled) {
		...
		for (MsgHeader header : headers) {
			if (Constants.VERP_BOUNCE_ADDR_XHEADER.equals(header.getName())) {
				...
				if (msgBean.getOrigRcpt() != null && !StringUtils.isEmpty(header.getValue())
				...
				if (ruleName == null) {
					...
					if (StringUtils.isEmpty(rPath) || "<>".equals(rPath.trim())) {
		...
		if (senderId != null) {
			...
			if (StringUtils.isEmpty(msgBean.getSenderId())) {
		...
		if (msgBean.getMsgRefId() != null && msgBean.getIsReceived()) {
			...
			EmailAddressVo addrVo = emailAddressDao.getToByMsgRefId(msgBean.getMsgRefId());
			if (addrVo != null) {
			...
			if (origVo == null) { // could be deleted by User or purged by Purge routine
				...
				if (StringUtils.isEmpty(msgBean.getSenderId())) {
					msgBean.setSenderId(origVo.getSenderId());
				}
				if (StringUtils.isEmpty(msgBean.getSubrId())) {
					msgBean.setSubrId(origVo.getSubrId());
				}
		...
		if (RuleNameEnum.HARD_BOUNCE.getValue().equals(ruleName)
			...
			if (StringUtils.isEmpty(msgBean.getFinalRcpt())
				...
				if (!StringUtils.isEmpty(finalRcpt)) {
		...
		try {
			while ((line = br.readLine()) != null) {
				...
				if (line.toLowerCase().startsWith("to:")) {
					...
					if (StringUtils.isEmpty(msgBean.getOrigSubject())) {
					...
					if (StringUtils.isEmpty(msgBean.getSmtpMessageId())) {
