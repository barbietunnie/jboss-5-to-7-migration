...
public class TaskScheduler {
	...
	public void scheduleTasks(MessageBean msgBean) throws DataValidationException,
		...
		RuleActionDao msgActionDao = (RuleActionDao) SpringUtil.getBean(factory, "ruleActionDao");
		List<RuleActionVo> actions = msgActionDao.getByBestMatch(msgBean.getRuleName(), null,
				msgBean.getSenderId());
		if (actions == null || actions.isEmpty()) {
			...
			AbstractTaskBo bo = (AbstractTaskBo) SpringUtil.getBean(factory, processBeanId);
			bo.process(msgBean);
			return;
		}
		for (int i = 0; i < actions.size(); i++) {
			RuleActionVo msgActionVo = actions.get(i);
			...
			if (processClassName != null && processClassName.trim().length() > 0) {
				...
				logger.info("scheduleTasks() - ProcessClassName [" + i + "]: "
						+ processClassName);
				try {
					bo = (AbstractTaskBo) Class.forName(processClassName).newInstance();
				}
				...
				bo = (AbstractTaskBo) SpringUtil.getBean(factory, processBeanId);
			}
			/*
			 * retrieve arguments
			 */
