...
public enum EmailVariableEnum implements EnumInterface {
	...
	SubscriberAddress(EmailVariableType.Custom,"Subscriber","StreetAddress",false,"",
			"SELECT CONTAC_WS(',',c.StreetAddress2,c.StreetAddress) as ResultStr FROM subscriber c, email_address e where e.EmailAddrId=c.EmailAddrId and e.EmailAddrId=?1","com.es.bo.external.SubscriberNameResolver"),
	...
	SubscriberStateCode(EmailVariableType.Custom,"Subscriber","StateCode",false,"",
			"SELECT CONTAC_WS(',',c.StateCode,c.ProvinceName) as ResultStr FROM subscriber c, email_address e where e.EmailAddrId=c.EmailAddrId and e.EmailAddrId=?1","com.es.bo.external.SubscriberNameResolver"),
