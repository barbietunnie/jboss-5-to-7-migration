...
public class RaveAescr1Test {
	static Logger logger = LoggerHelper.getLogger();

	@Autowired
	RaveAescr1Service service;
	@Autowired
	...
	@BeforeClass
	public static void RaveAescr1Prepare() {
	}
	...
	private String testRaveAescr1CaseNum1 = "test AE Case Num 1";
	private String testRaveAescr1Avet1 = "test event 1";
	private String testRaveAescr1Avet2 = "test event 2";
	
	@Test
	public void testItemGroupService() {
		// test insert
		RaveAescr1 id1 = new RaveAescr1();
		id1.setItemGroup(ig);
		id1.setAecasnum(testRaveAescr1CaseNum1);
		id1.setSerny("Yes");
		id1.setAevt(testRaveAescr1Avet1);

		if (ig.getRaveAescr1()==null) {
			ig.setRaveAescr1(id1);
		}
		igService.update(ig);

		RaveAescr1 ae0 = service.getByItemGroupId(ig.getId());
		assertNotNull(ae0);
		assertNotNull(ae0.getUpdtTime());
		assertTrue(testRaveAescr1Avet1.equals(ae0.getAevt()));
		...
		List<RaveAescr1> lst1 = service.getByAncestors(study.getStudyPK(), 
		...
		List<RaveAescr1> lst2 = service.getByCaseNumber(testRaveAescr1CaseNum1);
		...
		// test update
		ae0.setAevt(testRaveAescr1Avet2);
		service.update(ae0);
		RaveAescr1 id3 = service.getById(ae0.getId());
		assertTrue(testRaveAescr1Avet2.equals(id3.getAevt()));
