...
public class RaveSubjectTest {
	static Logger logger = LoggerHelper.getLogger();

	@Autowired
	RaveSubjectService service;
	@Autowired
	...
	@BeforeClass
	public static void RaveSubjectPrepare() {
	}
	...
	private String testRaveSubjectPtId1 = "test Patient Id 1";
	private String testRaveSubjectPt1 = "test patient 1";
	private String testRaveSubjectPt2 = "test patient 2";
	
	@Test
	public void testItemGroupService() {
		// test insert
		RaveSubject id1 = new RaveSubject();
		id1.setItemGroup(ig);
		id1.setPt_id(testRaveSubjectPtId1);
		id1.setPt(testRaveSubjectPt1);

		if (ig.getRaveSubject()==null) {
			ig.setRaveSubject(id1);
		}
		igService.update(ig);

		RaveSubject ae0 = service.getByItemGroupId(ig.getId());
		assertNotNull(ae0);
		assertNotNull(ae0.getUpdtTime());
		assertTrue(testRaveSubjectPtId1.equals(ae0.getPt_id()));
		assertTrue(testRaveSubjectPt1.equals(ae0.getPt()));
		...
		List<RaveSubject> lst1 = service.getByAncestors(study.getStudyPK(), 
		...
		List<RaveSubject> lst2 = service.getByPatientId(testRaveSubjectPtId1);
		...
		// test update
		ae0.setPt(testRaveSubjectPt2);
		service.update(ae0);
		RaveSubject id3 = service.getById(ae0.getId());
		assertTrue(testRaveSubjectPt2.equals(id3.getPt()));
		assertFalse(before.equals(id3.getUpdtTime()));
		assertNotNull(id3.getItemGroup());
		
		//List<RaveSubject> lst3 = service.getByStudyPK(study.getStudyPK());
		//assertFalse(lst3.isEmpty());
		
		RaveSubject id4 = service.getById(id1.getId());
		assertNotNull(id4);
