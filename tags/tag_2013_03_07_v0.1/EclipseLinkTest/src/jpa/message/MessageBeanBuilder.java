...
public final class MessageBeanBuilder {
	...
	private static String processEnvelope(Message msg, MessageBean msgBean, String toAddrDomain)
		...
		try {
			...
			while (enu.hasMoreElements()) {
				...
				if (XHeaderName.RETURN_PATH.getValue().equalsIgnoreCase(name)) {
		...
		try {
			...
			else if ((_froms = msg.getHeader(XHeaderName.RETURN_PATH.getValue())) != null && _froms.length > 0) {
		...
		// X-MAILER
		try {
			String[] hdrs = msg.getHeader(XHeaderName.MAILER.getValue());
			if (hdrs != null) {
				xmailer = hdrs;
		...
		try {
			String[] priority = ((MimeMessage) msg).getHeader(XHeaderName.PRIORITY.getValue());
			if (priority != null) {
	 ...
	 */
	public static MessageBean createMessageBean(MessageInbox msgVo) throws DataValidationException {
		if (isDebugEnabled)
		...
		try {
			...
			msgBean.setRuleName(logic.getRuleName());
		}
		catch (NoResultException e) {}
