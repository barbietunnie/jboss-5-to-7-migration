...
public class renderBoTest {
	...
	public void renderBoService() {
		...
		assertTrue(StringUtils.isNotBlank(req.getClientId()));
		assertTrue(Constants.DEFAULT_CLIENTID.equals(req.getClientId()));
		...
		assertTrue(req.getVariableOverrides().containsKey(ClientVariableEnum.ClientId.name()));
		...
		try {
			...
			assertTrue(req.getClientId().equals(rsp.getClientId()));
